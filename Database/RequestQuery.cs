...
namespace SPTC_APP.Database
{
    public static class RequestQuery
    {
        ...
        public static string GET_ALL_LOAN_INTEREST_IN_MONTH(int month, int year) => $"SELECT SUM({Field.PENALTIES}) FROM tbl_payment_details WHERE YEAR({Field.DATE}) = {year} AND MONTH({Field.DATE}) = {month} AND {Field.LEDGER_ID} <> -1 AND penalties > 0 AND ledger_type = \"LOAN\" || ledger_type = \"LONGTERMLOAN\"; ";
