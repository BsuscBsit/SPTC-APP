...
namespace SPTC_APP.Database
{
    public static class RequestQuery
    {
        ...
        public static string GET_ALL_PAYMENT_IN_MONTH(string table, int month, int year) =>
            $"SELECT SUM({Field.DEPOSIT}) FROM {Table.PAYMENT_DETAILS} WHERE YEAR({Field.DATE}) = {year} AND MONTH({Field.DATE}) = {month} AND {Field.LEDGER_TYPE} = \"{table}\" || {Field.REMARKS} = \"{table}\" AND {Where.ALL_NOTDELETED}";
        ...
        public static string GET_ALL_LOAN_PASTDUE_IN_MONTH(int month, int year) => $"SELECT SUM({Field.DEPOSIT}) FROM tbl_payment_details WHERE YEAR({Field.DATE}) = {year} AND MONTH({Field.DATE}) = {month} AND {Field.LEDGER_ID} <> -1 AND penalties > 0 AND ledger_type = \"LOAN\" || ledger_type = \"LONGTERMLOAN\"; ";
        public static string GET_ALL_LOAN_CURRENT_IN_MONTH(int month, int year) => $"SELECT SUM({Field.DEPOSIT}) FROM tbl_payment_details WHERE YEAR({Field.DATE}) = {year} AND MONTH({Field.DATE}) = {month} AND {Field.LEDGER_ID} <> -1 AND penalties = 0 AND ledger_type = \"LOAN\" || ledger_type = \"LONGTERMLOAN\"; ";
        public static string GET_ALL_LOAN_PENALTY_IN_MONTH(int month, int year) => $"SELECT SUM({Field.PENALTIES}) FROM tbl_payment_details WHERE YEAR({Field.DATE}) = {year} AND MONTH({Field.DATE}) = {month} AND {Field.LEDGER_ID} <> -1 AND penalties > 0 AND ledger_type = \"LOAN\" || ledger_type = \"LONGTERMLOAN\"; ";
        public static string GET_ALL_LOAN_INTEREST_IN_MONTH(int month, int year) => $"SELECT SUM(COALESCE(l.interest / l.terms_of_payment_month, 0) + COALESCE(l.interest / l.terms_of_payment_month, 0)) FROM tbl_payment_details AS pd\r\nLEFT JOIN tbl_loan_ledger AS l ON l.id = pd.ledger_id AND pd.ledger_type = 'LOAN' LEFT JOIN tbl_long_term_loan_ledger AS ltl ON ltl.id = pd.ledger_id AND pd.ledger_type = 'LONGTERMLOAN' WHERE pd.id <> -1 AND YEAR(pd.{Field.DATE}) = {year} AND MONTH(pd.{Field.DATE}) = {month};";
