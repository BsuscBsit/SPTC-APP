...
namespace SPTC_APP.View.Pages.Output
{
    public partial class ListReport : Window
    {
        ...
        public static string LOANS_SHORT(int month, int year) => $"SELECT * FROM {Table.LOAN} AS l LEFT JOIN {Table.FRANCHISE} as f ON l.{Field.FRANCHISE_ID} = f.{Field.ID} LEFT JOIN {Table.OPERATOR} AS o ON f.{Field.OPERATOR_ID} = o.{Field.ID} LEFT JOIN {Table.NAME} AS n ON o.{Field.NAME_ID} = n.{Field.ID} WHERE YEAR(l.{Field.DATE}) = {year} AND MONTH(l.{Field.DATE}) = {month} AND {Field.AMOUNT} > 0 AND l.isDeleted = 0 AND l.details = \"SHORT TERM\"";
        public static string LOANS_LONG(int month, int year) => $"SELECT * FROM {Table.LONG_TERM_LOAN} AS l LEFT JOIN {Table.FRANCHISE} as f ON l.{Field.FRANCHISE_ID} = f.{Field.ID} LEFT JOIN {Table.OPERATOR} AS o ON f.{Field.OPERATOR_ID} = o.{Field.ID} LEFT JOIN {Table.NAME} AS n ON o.{Field.NAME_ID} = n.{Field.ID} WHERE YEAR(l.{Field.DATE}) = {year} AND MONTH(l.{Field.DATE}) = {month} AND {Field.AMOUNT} > 0 AND l.isDeleted = 0";
        public static string LOANS_EMERGENCY(int month, int year) => $"SELECT * FROM {Table.LOAN} AS l LEFT JOIN {Table.FRANCHISE} as f ON l.{Field.FRANCHISE_ID} = f.{Field.ID} LEFT JOIN {Table.OPERATOR} AS o ON f.{Field.OPERATOR_ID} = o.{Field.ID} LEFT JOIN {Table.NAME} AS n ON o.{Field.NAME_ID} = n.{Field.ID} WHERE YEAR(l.{Field.DATE}) = {year} AND MONTH(l.{Field.DATE}) = {month} AND {Field.AMOUNT} > 0 AND l.isDeleted = 0 AND l.details = \"EMERGENCY\"";
